...
const AuthPage = () => {
  ...
  return (
    <div className="flex min-h-screen w-full bg-bg-primary text-text-primary font-['Plus_Jakarta_Sans']">
      {/* Left Panel */}
      <div className="hidden md:flex flex-col justify-between w-[40%] p-12 bg-bg-primary border-r border-border">
        <div>
          ...
          <p className="text-text-secondary font-medium opacity-80 mt-12 bg-bg-card inline-block px-4 py-2 rounded-full text-sm border border-border">
      ...
      {/* Right Panel */}
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 w-full bg-bg-secondary">
        <Card className="w-full max-w-md p-8! rounded-2xl! border border-border bg-bg-card shadow-xl" hoverable={false}>
          {/* Mobile Logo */}
          <div className="md:hidden flex items-center justify-center gap-2 mb-8">
            <div className="w-8 h-8 rounded bg-accent-primary flex items-center justify-center font-bold text-white">S</div>
          ...
          <div className="flex gap-6 mb-8 font-semibold border-b border-border">
            <button 
              className={`pb-3 cursor-pointer border-b-2 transition-colors ${mode === 'signup' ? 'text-white border-accent-primary' : 'text-text-secondary border-transparent hover:text-white'}`}
              onClick={() => { setMode('signup'); setErrors({}); setServerError(null); }}
            >
              Sign Up
            </button>
            <button 
              className={`pb-3 cursor-pointer border-b-2 transition-colors ${mode === 'login' ? 'text-white border-accent-primary' : 'text-text-secondary border-transparent hover:text-white'}`}
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            {mode === 'signup' && (
              <div>
                ...
                <input 
                  ...
                  className={`w-full bg-bg-primary border ${errors.name ? 'border-error' : 'border-border focus:border-accent-primary'} rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-accent-primary/30 text-text-primary placeholder:text-text-secondary transition-colors`}
                />
            ...
            <div>
              ...
              <input 
                ...
                className={`w-full bg-bg-primary border ${errors.email ? 'border-error' : 'border-border focus:border-accent-primary'} rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-accent-primary/30 text-text-primary placeholder:text-text-secondary transition-colors`}
              />
            ...
            <div>
              ...
              <div className="relative">
                <input 
                  ...
                  className={`w-full bg-bg-primary border ${errors.password ? 'border-error' : 'border-border focus:border-accent-primary'} rounded-lg px-4 py-3 text-sm focus:outline-none focus:ring-1 focus:ring-accent-primary/30 text-text-primary placeholder:text-text-secondary transition-colors`}
                />
